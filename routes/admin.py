...
def get_quota_limits_route(user_id):
    ...
    return jsonify(quota.compute_quota_status(user_id)), 200
...
def update_quota_limits_route(user_id):
    ...
    for field in _EDITABLE_LIMIT_FIELDS:
        ...
        database.relax_quota_limit(user_id, field, value)
        updated[field] = value
    ...
    if "tier" in payload:
        ...
        database.set_quota_tier(user_id, tier)
        updated["tier"] = tier
    ...
    if "cooldown_until" in payload:
        ...
        database.set_quota_cooldown(user_id, cooldown_value)
